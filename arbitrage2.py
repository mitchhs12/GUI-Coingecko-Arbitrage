...
historic_data = []
profit_threshold = 300 #Given so as to remove data outliers (expressed as a percentage e.g. 100 will remove profits > 100%)
...
pages = 2 #how many pages for each cryptocurrency are needed
...
def append(df,g):
    ...
    if g == False:
        del historic_data[(len(historic_data)-((len(historic_data)-1))):(len(historic_data)-1)]
        return historic_data
...
def call(coin_ticker):
    ...
    for q in range(call-1): #needs to be (call-1) because we already have the first page
        strung = str(q+2)
...
def df_loop():
    ...
    if historic_data[(len(historic_data)-1)-1]['Usd Price'].equals(historic_data[(len(historic_data)-1)]['Usd Price']):
        return df
    else:
        print("PRICES UPDATED")
        summary_statistics(historic_data[(len(historic_data)-1)]) #summary statistics on last df
...
def isolate_column(raw_data, column_name): #isolates and returns a column in a panda as a panda data frame
    list = []
...
def profit_maximiser():
    ...
    for coin_ticker in list_of_coins:
        ...
        time.sleep(0.7*pages) #otherwise I have to pay for Coingecko
    combined_dictionary = {"Coin Ticker":list_of_coins,"Expected Profit":profit_list}
    stats_dictionary = {'Marketcap Rank':[item for item in range(1,101)],'Coin Ticker':list_of_coins,\
...
def replace_column(data_frame, column_to_be_replaced, data_frame_2, column_data_frame_2): #replaces a column in a panda df with another
...
def summary_statistics_continue(df):
    ...
    index_min = df['Usd Price'].idxmin(skipna = True) #returns the index of the row of the lowest price
    index_max = df['Usd Price'].idxmax(skipna = True) #returns the index of the row of the highest price
    min_price_exchange = df.at[index_min,'Name'] #returns the exchange with the lowest price
    max_price_exchange = df.at[index_max,'Name'] #returns the exchange with the highest price
    min_price_vol = (df.at[index_min,'Usd Volume']) #returns the volume on the exchange that has the lowest price
    max_price_vol = (df.at[index_max,'Usd Volume']) #returns the volume on the exchange that has the highest price
    if min_price != max_price:
        while (min_price_vol < volume_threshold) or (np.isnan(min_price_vol)):
            if min_price_vol != max_price_vol:
                if (np.isnan(min_price_vol) and np.isnan(max_price_vol)):
                    ...
                    stats_df = min_price, (df.at[index_min,'Target']), max_price, df.at[index_max,'Target'], index_min+1, index_max+1, \
        min_price_exchange, min_price_vol, max_price_exchange, max_price_vol, percentage_profit
                    return percentage_profit, stats_df
                else:
                    df.at[index_min, 'Usd Price'] = np.NaN
                    min_price = df['Usd Price'].min(skipna = True)
                    index_min = df['Usd Price'].idxmin(skipna = True) #returns the row of the lowest price
        ...
        min_price_exchange, min_price_vol, max_price_exchange, max_price_vol, percentage_profit
                    ...
                    index_max = df['Usd Price'].idxmax(skipna = True) #Returns the row of the highest price
                    max_price_vol = (df.at[index_max,'Usd Volume']) #Returns the volume on the exchange that has the highest price
                    max_price_exchange = df.at[index_max,'Name'] #Returns the exchange with the highest price
            else:
                break
    ...
    print('Assuming Buy and Sell at the lowest and highest price:', percentage_profit,'%')
...
expected_profit_df.mask(expected_profit_df['Expected Profit'] >= profit_threshold, np.NaN, inplace=True)
...
#button logic
tableSelMenu = tk.OptionMenu(frame_buttons, tableSelBtn, 'Most Profitable Coin', 'All Statistics', 'Expected Profits')
